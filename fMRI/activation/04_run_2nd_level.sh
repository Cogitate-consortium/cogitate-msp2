...
# Slurm 2nd-level activation FEAT. Run after 03_make_fsf_files.sh and
# 02_create_identity_reg_for_2nd_lvl.sh; then 06_make_fsf_files_3rdlevel.sh (cope inputs)
# (before 06_make_fsf_files_3rdlevel.sh).
#
...
# report under job_files/run_activation_2nd_level/.
#
# Usage: bash 04_run_2nd_level.sh
...
JOB_FILES_ROOT="${ACTIVATION_CODE_ROOT}/job_files/run_activation_2nd_level"
SLURM_TEMPLATE="${ACTIVATION_CODE_ROOT}/slurm_templates/ACTIVATION_2nd_lvl.sh"
...
declare -a ANALYSES=(
    "seen_face_vs_unseen_face:analysis-2ndGLM_seen_face_vs_unseen_face_space-MNI152NLin2009cAsym:ACTIVATION_min_sf_uf"
    "seen_object_vs_unseen_object:analysis-2ndGLM_seen_object_vs_unseen_object_space-MNI152NLin2009cAsym:ACTIVATION_min_so_uo"
)
...
{
    echo "# Activation 2nd-level — 1st-level run inclusion report (all subjects)"
...
append_submission_status() {
    ...
    local status="$3"
    {
        echo "## submission_update analysis=${analysis_label}\tsubject=sub-${subj}\tsubmission_status=${status}"
        echo ""
    } >> "$RUNS_REPORT"
}
...
append_run_report_status() {
    ...
    local status="$3"
    {
        echo "## analysis=${analysis_label}\tsubject=sub-${subj}\tstatus=${status}"
...
for analysis_spec in "${ANALYSES[@]}"; do
    IFS=':' read -r analysis_label analysis_suffix filter_mode <<< "$analysis_spec"
    ...
    if [ ${#SUBJECTS[@]} -eq 0 ]; then
        echo "ERROR: no subjects for ${analysis_label} (${filter_mode})" >&2
        exit 1
    fi

    echo ">> ${analysis_label}: subjects=${#SUBJECTS[@]} (filter: ${filter_mode})"
    ...
    for subj in "${SUBJECTS[@]}"; do
        ...
        slurm_script="${job_dir}/slurm_${subj_id}_${analysis_label}.sh"
        base_fsf="${fsf_dir}/sub-${subj}_ses-V2_task-VG_${analysis_suffix}.fsf"
        patched_fsf="${job_dir}/sub-${subj}_ses-V2_task-VG_${analysis_suffix}_included_runs.fsf"
        feat_log="${job_dir}/feat_${subj_id}_${analysis_label}.log"
        slurm_out="${job_dir}/slurm_${subj_id}_${analysis_label}_%j.out"

        if [ -f "$run_marker" ]; then
            n_skipped=1
            append_run_report_status "$analysis_label" "$subj" "skipped_marker_exists"
            echo ">> ${subj_id} ${analysis_label}: submitted=0 skipped=1 preflight_failed=0 sbatch_failed=0"
            continue
        fi

        if [ ! -f "$base_fsf" ]; then
            echo "ERROR: ${subj_id} ${analysis_label}: missing FSF file: ${base_fsf} (run 03_make_fsf_files.sh?)" >&2
            n_preflight_failed=1
            append_run_report_status "$analysis_label" "$subj" "missing_base_fsf"
            echo ">> ${subj_id} ${analysis_label}: submitted=0 skipped=0 preflight_failed=1 sbatch_failed=0"
            continue
        fi

        _prep_tmp="$(mktemp)"
        if ! prepare_fsf_for_good_runs "$subj" "$analysis_label" "$base_fsf" "$patched_fsf" "preflight_ok" >"$_prep_tmp" 2>&1; then
            cat "$_prep_tmp" >&2
            rm -f "$_prep_tmp"
            echo "ERROR: ${subj_id} ${analysis_label}: could not build FSF from successful 1st-level runs (see ${RUNS_REPORT})" >&2
            n_preflight_failed=1
            echo ">> ${subj_id} ${analysis_label}: submitted=0 skipped=0 preflight_failed=1 sbatch_failed=0"
            continue
        fi
        ...
        echo ">> ${subj_id} ${analysis_label}: using ${n_included} run(s) (${included_list})"

        cat > "$run_marker" <<EOF
# Activation 2nd-level FEAT — marker / submission + job summary
status=SUBMITTED
...
EOF
        {
            awk '/^### ACTIVATION_EXPORT_BLOCK_INSERT ###$/ {exit} {print}' "$SLURM_TEMPLATE"
            printf '#SBATCH --output=%s\n' "$slurm_out"
            printf '#SBATCH --error=%s\n' "$slurm_out"
            printf 'export RUN_MARKER=%q\n' "$run_marker"
            printf 'export FEAT_LOG=%q\n' "$feat_log"
            printf 'export FSF_FILE=%q\n' "$fsf_file"
            awk '/^### ACTIVATION_EXPORT_BLOCK_INSERT ###$/ {f=1; next} f' "$SLURM_TEMPLATE"
        } > "$slurm_script"
        ...
        if ! job_out=$(sbatch "$slurm_script" 2>&1); then
            ...
            n_sbatch_failed=1
            append_submission_status "$analysis_label" "$subj" "sbatch_failed"
        else
            {
                echo ""
            ...
            n_submitted=1
            append_submission_status "$analysis_label" "$subj" "submitted"
        fi

        echo ">> ${subj_id} ${analysis_label}: submitted=${n_submitted} skipped=${n_skipped} preflight_failed=${n_preflight_failed} sbatch_failed=${n_sbatch_failed}"
    done
done
