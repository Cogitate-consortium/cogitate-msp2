...
# After 2nd-level activation FEAT (.gfeat), create dummy reg/ at each subject's 2nd-level
# .gfeat root (legacy path for inputtype 1 + parent .gfeat inputs). Current activation
# 3rd level uses inputtype 2 (cope/varcope .nii.gz) and does not call this script.
# Run after 04_run_2nd_level.sh if reverting to parent-.gfeat 3rd-level FSFs.
#
# Usage: bash 05_create_identity_reg_for_3rd_lvl.sh
#
# Cluster rerun (activation 3rd level): 05 -> 06 -> delete group .gfeat (or
# delete_fslfeat_derivatives.py --analysis activation --level 3rd --execute) ->
# clear job_files/run_activation_3rd_level/*/*.run -> 07 -> 08_overlap.sh
#
...
# label:gfeat_suffix:csv_filter_column:cope_idx
declare -a ANALYSES=(
    "seen_face_vs_unseen_face:analysis-2ndGLM_seen_face_vs_unseen_face_space-MNI152NLin2009cAsym:ACTIVATION_min_sf_uf:1"
    "seen_object_vs_unseen_object:analysis-2ndGLM_seen_object_vs_unseen_object_space-MNI152NLin2009cAsym:ACTIVATION_min_so_uo:2"
)
...
SUBJECT_CSV="$SUBJECT_CSV" ANALYSES_CSV="$ANALYSES_CSV" python3 <<'PY' >"$_sublist_tmp"
import os
import sys
...
for spec in analyses:
    _label, _suffix, col, _cope = spec.split(":", 3)
    if col not in subj_df.columns:
        print(f"ERROR: missing filter column {col} in {path}", file=sys.stderr)
        sys.exit(1)

for spec in analyses:
    label, suffix, col, cope_s = spec.split(":", 3)
    subs = subj_df.loc[subj_df[col].astype(str).str.upper().eq("TRUE"), "sub_code"]
    for sub in subs:
        print(f"{sub}\t{label}\t{suffix}\t{cope_s}")
PY
_py_st=$?
if [ "$_py_st" -ne 0 ]; then
    rm -f "$_sublist_tmp"
...
while IFS=$'\t' read -r sub_code analysis_label gfeat_suffix cope_s || [ -n "${sub_code}" ]; do
    ...
    for ext in .gfeat .feat; do
        candidate="${FSLFEAT_ROOT}/activation/${subj_id}/${parent_stem}${ext}"
        if [ -d "$candidate" ]; then
