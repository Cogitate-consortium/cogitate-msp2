...
Modified by Yamil Vidal 07/10/2023
@author: Yamil Vidal
...
def run_conf_for_all_subjects(subjects, session_labels, confounds_of_interest):
    ...
    for sub in subjects:
        ...
        for ses in session_labels:
            ...
            if len(runs) == 0:
                # Try with alternative pattern for desc-confounds_regressors.tsv
                alt_input_fname = input_fname.replace('desc-confounds_timeseries.tsv', 'desc-confounds_regressors.tsv')
                runs_alt = glob.glob(alt_input_fname)
                if len(runs_alt) == 0:
                    print('!!! No confounds_timeseries.tsv or confounds_regressors.tsv found for: ' + sub  + ' | Session: ' + ses + ' !!!')
                    continue
                else:
                    runs = runs_alt
                    print('... Using confounds_regressors.tsv for: ' + sub  + ' | Session: ' + ses)
            ...
            for run in runs:
                ...
                # Use the discovered confounds file path directly (can be either
...
if __name__ == '__main__':
    ...
    # Filter out participants that have FALSE in all analysis columns
    meta_cols = {'sub_code', 'modality', 'Lab'}
    analysis_cols = [c for c in subj_df.columns if c not in meta_cols]
    if analysis_cols:
        analysis_flags = subj_df[analysis_cols].apply(lambda s: s.astype(str).str.upper().eq('TRUE'))
        subj_df = subj_df.loc[analysis_flags.any(axis=1)]

    subjects = subj_df['sub_code'].values

    # One subject for debugging
    # subjects = np.array(['SC160'])

    print('Number of Subjects:', subjects.size)
    
    subjects = ['sub-' + s for s in subjects ]
