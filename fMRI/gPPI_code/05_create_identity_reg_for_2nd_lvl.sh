...
# After 1st-level PPI FEAT with registration off (fMRIPrep MNI BOLD), create a
# dummy reg/ in each run-level .feat so 2nd-level FEAT can merge inputs without
# re-registering. No FLIRT/FNIRT is run; func space is treated as standard space.
#
# Usage: bash 05_create_identity_reg_for_2nd_lvl.sh
#
...
JOB_FILES_ROOT="${GPPI_CODE_ROOT}/job_files/create_identity_reg_for_2nd_lvl"
...
for subj in "${SUBJECTS[@]}"; do
    ...
    for analysis in "${ANALYSES[@]}"; do
        for i_run in "${RUNS[@]}"; do
            feat_analysis="${subj_id}_ses-V2_task-VG_run-${i_run}_analysis-1stGLM_${analysis}-MNI152NLin2009cAsym.feat"
            feat_path="${FSLFEAT_ROOT}/gPPI/${subj_id}/${feat_analysis}"
            reg_marker="${job_dir}/${subj_id}_ses-V2_task-VG_run-${i_run}_analysis-1stGLM_${analysis}-MNI152NLin2009cAsym_identity_reg.run"
