...
### GPPI_3RD_EXPORT_BLOCK_INSERT ###
# 10_run_PPI_3rd_level.sh inserts #SBATCH --output/--error and exports before the body runs.

module load FSL
module load SciPy-bundle/2024.05-gfbf-2024a

export OPENBLAS_NUM_THREADS=1
# Already inside Slurm: run gfeatprep/FEAT substeps in-process (avoids fsl_sub _@_ errors).
...
[ -f "${FSF_FILE}" ] || preflight_fail "missing FSF file: ${FSF_FILE} (run 09_make_fsf_files_3rdlevel.sh?)"
...
while IFS= read -r line || [ -n "${line}" ]; do
  ...
  if [[ "${path}" =~ /cope[0-9]+\.feat$ ]]; then
    ...
    if [[ ! -f "${parent_gfeat}/reg/example_func2standard.mat" ]]; then
      missing_gfeat_reg="${missing_gfeat_reg}${missing_gfeat_reg:+$'\n'}  missing 2nd-level identity reg: ${parent_gfeat}/reg/example_func2standard.mat (run 08_create_identity_reg_for_3rd_lvl.sh?)"
    fi
  fi
done < <(grep -E '^set feat_files\(' "${FSF_FILE}" 2>/dev/null || true)
