...
# 07_run_PPI_2nd_level_VG.sh inserts #SBATCH --output/--error and exports before the body runs.
...
while IFS= read -r line || [ -n "${line}" ]; do
  ...
  if [[ "${path}" == *.feat && ! -f "${path}/reg/example_func2standard.mat" ]]; then
    missing="${missing}${missing:+$'\n'}  missing identity reg: ${path}/reg/example_func2standard.mat (run 05_create_identity_reg_for_2nd_lvl.sh?)"
  fi
...
set -e
{
  ...
  echo "---- last 150 lines of ${FEAT_LOG} ----"
