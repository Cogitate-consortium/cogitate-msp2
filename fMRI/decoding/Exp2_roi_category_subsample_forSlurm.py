...
num_iter = 10
condition = 'all'  # 'all' or 'seen' or 'unseen'
...
def prepare_nibetaseries_data(vg_or_replay, nibetaseries_filename, tsv_filename, condition, stimulus_categories, number_of_runs):
    ...
    for run in unique_runs:
        ...
        for filename in nibetaseries_filename:
            if filename.find('ses-V1') == -1:#older V1 files exist in these folders!
...
print('using list: DECODING_min_sf_so_uf_uo')
...
def run_analysis():
                            ...
                            if approach == 'generalization':
                                ...
                                if num_unselected < num_selected:  # need to subsample
                                    ...
                                    for i_iter in range(num_iter):
                                        ...
                                        subsample_scores.append(roi_decoding_crossval(sampled_beta_maps, sampled_trial_labels, sampled_run_labels,
    ...
    df.to_csv(os.path.join(output_dir, 'roi_' + vg_or_replay + '_category_' + condition + '_' + str(n_voxels) + 'vox_' + roi_condition + '_' + str(num_iter) + 'subsample.csv'))
