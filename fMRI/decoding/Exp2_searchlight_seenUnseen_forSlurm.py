...
output_dir = os.path.join('/mnt/beegfs/XNAT/COGITATE/fMRI/phase_2/processed/bids/derivatives/decoding/nibetaseries/ses-V2/',
                          'searchlight_decoding_seenUnseen/')
...
def prepare_nibetaseries_data(nibetaseries_filename, tsv_filename, condition, stimulus_categories, number_of_runs):
    ...
    for run in unique_runs:
        ...
        trial_labels = trial_labels + [seen_labels[0]] * sum(selected_stim_1_Face) + [seen_labels[1]] * sum(selected_stim_2_Face)
        run_labels = run_labels + [run] * len([seen_labels[0]] * sum(selected_stim_1_Face) + [seen_labels[1]] * sum(selected_stim_2_Face))
        trial_labels = trial_labels + [seen_labels[0]] * sum(selected_stim_1_Object) + [seen_labels[1]] * sum(selected_stim_2_Object)
        run_labels = run_labels + [run] * len([seen_labels[0]] * sum(selected_stim_1_Object) + [seen_labels[1]] * sum(selected_stim_2_Object))
...
def single_subject_analysis():
    ...
    vg_or_replay = 'VG'
    print('running seen vs. unseen decoding: ' + sub_code)
    ...
    for root, sess_dirs, session_files in os.walk(os.path.join(nibetaseries_dir, sub)):
        ...
        for sess_directory in sess_dirs:
            if (sess_directory.find('ses-V2') != -1):
                ...
                output_filename = 'searchlight_' + sub + '_' + vg_or_replay + '_seenUnseen.nii'
