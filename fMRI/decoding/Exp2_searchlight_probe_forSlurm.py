"""
Date created:  10-19-2023 by Zvi
"""
import os
import operator
import pandas as pd
...
def prepare_nibetaseries_data(nibetaseries_filename, tsv_filename, condition, stimulus_categories, number_of_runs):
    ...
    for run in unique_runs:
        ...
        selected_stim_2 = stim_all.type == 'GAME_STIMULUS'  # ALL unprobed
        if condition == 'seen':
            selected_stim_1 = (stim_all.response == seen_response) & (stim_all.type == 'GAME_PROBE')  # SEEN probe
        elif condition == 'unseen':
            selected_stim_1 = (stim_all.response == unseen_response) & (stim_all.type == 'GAME_PROBE')  # UNSEEN probe
        elif condition == 'all':
            selected_stim_1 = stim_all.type == 'GAME_PROBE'  # ALL probed
        else:
        ...
        for filename in nibetaseries_filename:
            if filename.find('ses-V1')==-1:#older ses-V1 files exist in these folders!
                # for Face trials, add betas corresponding to Probe trials, and to No-probe trials
                if filename.find(vg_or_replay + '_run-' + str(run + 1) + '_space-MNI152NLin2009cAsym_desc-' +
                                 stimulus_categories[0] + '_betaseries.nii.gz') != -1:
                    # selected_stim_1_Face = probe_boolean[behavioral.trial_type == stimulus_categories[0]]
                    ...
                    # selected_stim_2_Face = noprobe_boolean[behavioral.trial_type == stimulus_categories[0]]
                ...
                # for Object trials, add betas corresponding to Probe trials, and to No-probe trials
                if filename.find(vg_or_replay + '_run-' + str(run + 1) + '_space-MNI152NLin2009cAsym_desc-' +
                                 stimulus_categories[1] + '_betaseries.nii.gz') != -1:
                    # selected_stim_1_Object = probe_boolean[behavioral.trial_type == stimulus_categories[1]]
                    ...
                    beta_maps_list.append(nibetaseries_stim_1_files)
                    # selected_stim_2_Object = noprobe_boolean[behavioral.trial_type == stimulus_categories[1]]
        ...
        trial_labels = trial_labels + [probe_labels[0]] * sum(selected_stim_1_Face) + [probe_labels[1]] * sum(selected_stim_2_Face)
        run_labels = run_labels + [run] * len([probe_labels[0]] * sum(selected_stim_1_Face) + [probe_labels[1]] * sum(selected_stim_2_Face))
        trial_labels = trial_labels + [probe_labels[0]] * sum(selected_stim_1_Object) + [probe_labels[1]] * sum(selected_stim_2_Object)
        run_labels = run_labels + [run] * len([probe_labels[0]] * sum(selected_stim_1_Object) + [probe_labels[1]] * sum(selected_stim_2_Object))
...
def single_subject_analysis():
    ...
    parser.add_argument('--condition', type=str, default=None,
                        help="seen, unseen, all, seen-seen, seen-unseen, unseen, etc.")
    args = parser.parse_args()
    sub_code = args.subject
    condition = args.condition
    sub = 'sub-' + sub_code

    print('running probe decoding : ' + sub_code + '  ' + vg_or_replay + ' ' + condition)
    ...
    if (vg_or_replay == 'Replay') | (vg_or_replay == 'VG'):
        ...
        raise Exception(
            "unrecognized vg_or_replay value, should be 'VG', 'Replay', 'Replay-VG', or 'VG-Replay'. Received: " + vg_or_replay)
    ...
    for root, sess_dirs, session_files in os.walk(os.path.join(nibetaseries_dir, sub)):
        ...
        for sess_directory in sess_dirs:
            if (sess_directory.find('ses-V2') != -1):
                ...
                output_filename = 'searchlight_' + sub + '_probe_' + condition + '_' + approach + '.nii'
