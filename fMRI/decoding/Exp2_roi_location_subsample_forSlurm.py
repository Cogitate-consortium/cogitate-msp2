...
def prepare_nibetaseries_data(vg_or_replay, nibetaseries_filename, tsv_filename, condition, stimulus_categories, number_of_runs):
    ...
    for run in unique_runs:
        ...
        stim_all = behavioral
        location_1_boolean = (stim_all.stimulusLocation == stimulus_location[0]) | (stim_all.stimulusLocation == stimulus_location[1])
        location_2_boolean = (stim_all.stimulusLocation == stimulus_location[2]) | (stim_all.stimulusLocation == stimulus_location[3])
        if (vg_or_replay == 'Replay') & (condition == 'seen'):
            selected_stim_1 = (stim_all.response == Replay_seen_response) & location_1_boolean  # NO-GO SEEN left
            selected_stim_2 = (stim_all.response == Replay_seen_response) & location_2_boolean  # NO-GO SEEN right
            ...
            unselected_stim_1 = (stim_all.response == unseen_response) & location_1_boolean # UNSEEN left
            unselected_stim_2 = (stim_all.response == unseen_response) & location_2_boolean # UNSEEN right
            ...
            # collect trials for UNSEEN, to subsample
            unselected_stim_1 = (stim_all.response == seen_response) & location_1_boolean # SEEN left
            unselected_stim_2 = (stim_all.response == seen_response) & location_2_boolean # SEEN right
        elif condition == 'all':
        ...
        for filename in nibetaseries_filename:
            if filename.find('ses-V1')==-1:#older ses-V1 files exist in these folders!
                ...
                if filename.find(vg_or_replay + '_run-' + str(run + 1) + '_space-MNI152NLin2009cAsym_desc-' +
                    ...
                    # find unselected trials for subsampling
                    unselected_stim_1_Face = (unselected_stim_1[behavioral.trial_type == stimulus_categories[0]])
                    unselected_stim_2_Face = (unselected_stim_2[behavioral.trial_type == stimulus_categories[0]])
                ...
                if filename.find(vg_or_replay + '_run-' + str(run + 1) + '_space-MNI152NLin2009cAsym_desc-' +
                    ...
                    beta_maps_list.append(nibetaseries_stim_2_files)
                    # find unselected trials for subsampling
                    unselected_stim_1_Object = (unselected_stim_1[behavioral.trial_type == stimulus_categories[1]])
                    unselected_stim_2_Object = (unselected_stim_2[behavioral.trial_type == stimulus_categories[1]])
        ...
        unselected_trial_labels = unselected_trial_labels + [location_labels[0]] * sum(unselected_stim_1_Face) + [location_labels[1]] * sum(unselected_stim_2_Face)
        unselected_run_labels = unselected_run_labels + [run] * len([location_labels[0]] * sum(unselected_stim_1_Face) + [location_labels[1]] * sum(unselected_stim_2_Face))
        unselected_trial_labels = unselected_trial_labels + [location_labels[0]] * sum(unselected_stim_1_Object) + [location_labels[1]] * sum(unselected_stim_2_Object)
        unselected_run_labels = unselected_run_labels + [run] * len([location_labels[0]] * sum(unselected_stim_1_Object) + [location_labels[1]] * sum(unselected_stim_2_Object))
...
def run_analysis():
                    ...
                    if (sess_directory.find('ses-V2') != -1):
                        ...
                        if use_evc_rois:
                            if os.path.exists(evcRoiPath):
                                for filecnt, filename in enumerate(os.listdir(evcRoiPath)):
                                    if ((filename.find('.nii.gz') != -1) & (
                                            filename.find(stimulus_categories[1].lower()) != -1) & (
                                                filename.find('_V1') != -1) & filename.find(
                                        str(n_voxels) + '_') != -1) & (
                        ...
                        if os.path.exists(roiPath):
                            for filecnt, filename in enumerate(os.listdir(roiPath)):
                            ...
                            if approach == 'generalization':
                                ...
                                for filecnt, filename in enumerate(os.listdir(os.path.join(bids_dir, sub, sess_directory, 'func'))):
                                    if (filename.find('.tsv') != -1) and (filename.find(vg_or_replay_list[0]) != -1):
                                        tsv_filename1.append(os.path.join(bids_dir, sub, sess_directory, 'func', filename))
                                for filecnt, filename in enumerate(os.listdir(os.path.join(bids_dir, sub, sess_directory, 'func'))):
                                    if (filename.find('.tsv') != -1) and (filename.find(vg_or_replay_list[1]) != -1):
                                ...
                                if num_unselected1+num_unselected2 < num_selected1+num_selected2:  # need to subsample
                                    ...
                                    for i_iter in range(num_iter):
                                        ...
                                        if num_unselected1 < num_selected1: #subsample training data
                                        ...
                                        if num_unselected2 < num_selected2: # subsample testing data
                                            sampled_ind2 = sample(range(num_selected2), num_unselected2)
                                        ...
                                        subsample_scores.append(
                                            roi_decoding_generalization(sampled_beta_maps1, sampled_trial_labels1,
                                                                        sampled_run_labels1,
                                                                        sampled_beta_maps2, sampled_trial_labels2,
                                                                        sampled_run_labels2,
                                                                        classifier, func_roi_filename))
                                    roi_accuracy_scores = np.mean(subsample_scores, axis=0)  # mean over iterations
                                else:
                                    roi_accuracy_scores = roi_decoding_generalization(beta_maps1, trial_labels1, run_labels1,
                                                                                      beta_maps2, trial_labels2, run_labels2,
                                                                                      classifier, func_roi_filename)
                            elif approach == 'within_condition':

                                # Loop over event tsv files of the subject
                                for filecnt, filename in enumerate(os.listdir(os.path.join(bids_dir, sub, sess_directory, 'func'))):
                                    if (filename.find('.tsv') != -1) and (filename.find(vg_or_replay) != -1):
                                ...
                                beta_maps, trial_labels, run_labels, num_selected, num_unselected = prepare_nibetaseries_data(
                                    vg_or_replay, nibetaseries_filename, tsv_filename, condition, stimulus_categories,
                                    number_of_runs)
                                ...
                                if num_unselected < num_selected:  # need to subsample
                                    ...
                                    for i_iter in range(num_iter):
                                        ...
                                        subsample_scores.append(
                                            roi_decoding_crossval(sampled_beta_maps, sampled_trial_labels, sampled_run_labels,
                                                                  classifier, func_roi_filename))
                                    roi_accuracy_scores = np.mean(subsample_scores, axis=0)  # mean over iterations
                            ...
                            print('MISSING ROIS, excluding subject: ' + sub_code)
    ...
    df.to_csv(os.path.join(output_dir, 'roi_' + vg_or_replay + '_location_' + condition + '_' + str(n_voxels) + 'vox_' + roi_condition + '_' + str(num_iter) + 'subsample.csv'))
