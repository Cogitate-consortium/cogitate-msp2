...
Command: python3 Exp2_searchlight_RSA_Replay_category_callSlurm.py
"""
...
def run_analysis():
    ...
    for sub_code, analysis_check in zip(subject_list, analysis_check_list):
        ...
        if analysis_check:
            #slurm command
            run_command = f"sbatch Exp2_searchlight_RSA_Replay_category.sh --subject={sub_code}"
    ...
    toc = time.time()
    print('subject list: DECODING_min_sf_so')
