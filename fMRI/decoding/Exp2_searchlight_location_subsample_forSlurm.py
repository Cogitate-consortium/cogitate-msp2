...
Purpose: Runs searchlight on single subjects, decoding location. To be called by Slurm:
    ...
    for root, sess_dirs, session_files in os.walk(os.path.join(nibetaseries_dir, sub)):
        ...
        for sess_directory in sess_dirs:
            if (sess_directory.find('ses-V2') != -1):
                ...
                if approach == 'generalization':
                    ...
                    if num_unselected < num_selected:  # need to subsample
                        ...
                        for i_iter in range(num_iter):
                            ...
                            temp_img = (searchlight_decoding_crossval(sampled_beta_maps, sampled_trial_labels,
                                                                      sampled_run_labels,
                                                                      classifier, searchlight_radius,
                                                                      searchlight_neighbours_matrix, masker))
                            subsample_img.append(temp_img)
                        searchlight_img_orig = nilearn.image.mean_img(subsample_img)
                ...
                output_filename = 'searchlight_' + sub + '_' + vg_or_replay + '_location_' + condition + '_' + approach + '_' + str(num_iter) + 'subsample.nii'
