...
Purpose: combines maps from group of subjects
decoding_problem: category or location
vg_or_replay: VG or Replay, or VG-Replay, or VG-VG, etc.
Condition: seen or unseen, or seen-unseen, or unseen-unseen, etc.
Command: python3 Exp2_searchlight_group_forSlurm.py --decoding_problem "${decoding_problem}" --condition "${condition}" --vg_or_replay "${vg_or_replay}"
"""
...
def second_level_display(logp_map, accuracy_map, chance_level, searchlight_dir, output_filename):
...
def run_analysis():
    ...
    parser.add_argument('--vg_or_replay', type=str, default=None,
                        help="VG, Replay, VG-Replay, Replay-Replay, VG-VG, etc.")
    ...
    output_dir = os.path.join(
        ...
        'searchlight_decoding_' + decoding_problem + '/')
    ...
    for sub_code, analysis_check in zip(subject_list, analysis_check_list):
        ...
        if analysis_check:
            sub = 'sub-' + sub_code
            output_filename = 'searchlight_' + sub + '_' + vg_or_replay + '_' + decoding_problem + '_' + condition + '_' + approach + '.nii'
    ...
    output_filename = 'searchlight_group_' + vg_or_replay + '_' + decoding_problem + '_' + condition + '_' + approach
    second_level_display(stats_map, accuracy_map, chance_level, output_dir, output_filename)
