...
print('using list: DECODING_min_sl_sr')
...
def run_analysis():
                            ...
                            if os.path.exists(evcRoiPath):
                                for filecnt, filename in enumerate(os.listdir(evcRoiPath)):
                                    if ((filename.find('.nii.gz') != -1) & (
                                            ...
                                            filename.find('_V1') != -1) & filename.find(str(n_voxels) + '_') != -1) & (bool([selected_roi_list.index(x) for x in selected_roi_list if x in filename])):
                                        func_roi_filename.append(os.path.join(func_rois_dir, sub, filename))
                                        index = [selected_roi_list.index(x) for x in selected_roi_list if x in filename] #finds the index in selected_roi_list that correspond to filename
                        ...
                        if os.path.exists(roiPath):
                            for filecnt, filename in enumerate(os.listdir(os.path.join(func_rois_dir, sub))):
                                ...
                                if ((filename.find('.nii.gz') != -1) & (filename.find(stimulus_categories[0].lower()) != -1) & (
                                        ...
                                        filename.find('_' + str(n_voxels) + '_') != -1) & (bool([selected_roi_list.index(x) for x in selected_roi_list if x in filename]))):
                                    func_roi_filename.append(os.path.join(func_rois_dir, sub, filename))
                            ...
                            if (len(func_roi_filename) > 0) & (sub != 'sub-SD131') & (sub != 'sub-SC140') & (sub != 'sub-SD193'):
                                if approach == 'generalization':
                                    ...
                                    func_roi_filename = [x for _, x in sorted(zip(roi_labels, func_roi_filename))]#sort func_roi_filename according to roi_labels
                                    ...
                                    print("Running " + sub)
                                    beta_maps, trial_labels, run_labels = prepare_nibetaseries_data(
    ...
    df.to_csv(os.path.join(output_dir, 'roi_' + vg_or_replay + '_location_' + condition + '_' + approach + '_' + str(n_voxels) + 'vox_' + roi_condition + '.csv'))
