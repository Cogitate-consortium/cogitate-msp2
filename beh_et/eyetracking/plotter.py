...
def make_it_rain(data, cols, col_names, color_list, plot_title, plot_x_label, plot_y_label, sub_line, horizontal_lines,
    ...
    for i in range(len(cols)):
        ...
        plt.scatter(x=scat_x, y=data_c, marker="o", color=color_list[i], alpha=0.5, edgecolor=color_list[i], s=45)
    ...
    if custom_ylim is not None:
        ...
        yticks_labels.extend([f"{x}" for x in np.arange(0, upper_lim, skip)])
    else:
