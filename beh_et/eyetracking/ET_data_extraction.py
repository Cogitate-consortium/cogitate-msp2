...
OVERLAP = 'msOverlap'
IS_PROBED = "probed"
SACC_FEATURES = {THRESHOLD: 1,  # upper cutoff for microsaccades (in degrees)
...
def dist_from_target(trial_info, et_data_prepro, params):
    """
    ...
    trial_info[COORDS] = trial_info[DataParser.STIM_LOC].apply(lambda row: list(params[COORDS][row]))

    for ind, trial in trial_info.iterrows():
        # gaze distance from target stimulus in trial
        mask = (et_data_prepro[DataParser.DF_SAMPLES][DataParser.TRIAL] == trial["trialNumber"])
        valid = et_data_prepro[DataParser.DF_SAMPLES][mask]
        # dist from stimulus in trial in PIXELS
        et_data_prepro[DataParser.DF_SAMPLES].loc[mask, f"{eye}StimDistPixels"] = np.sqrt(((valid[f"{eye}X"] - trial[COORDS][0]) ** 2) + ((valid[f"{eye}Y"] - trial[COORDS][1]) ** 2))
        # gaze distance from center in trial in PIXELS
        et_data_prepro[DataParser.DF_SAMPLES].loc[mask, f"{eye}CenterDistPixels"] = np.sqrt(((valid[f"{eye}X"] - center_coords[0]) ** 2) + ((valid[f"{eye}Y"] - center_coords[1]) ** 2))

        for coordi in ['X', 'Y']:
            center_coord = 0 if coordi == 'X' else 1
            et_data_prepro[DataParser.DF_SAMPLES].loc[mask, f"{eye}{coordi}StimDistPixelsSigned"] = valid[f"{eye}{coordi}"] - trial[COORDS][center_coord]  # in pixels
            et_data_prepro[DataParser.DF_SAMPLES].loc[mask, f"{eye}{coordi}StimDistDegsSigned"] = et_data_prepro[DataParser.DF_SAMPLES].loc[mask, f"{eye}{coordi}StimDistPixelsSigned"] * params['DegreesPerPix']  # in degrees

    # Convert distance in pixels to degrees
    mask = (et_data_prepro[DataParser.DF_SAMPLES][DataParser.TRIAL] != -1)  # Go over only samples that are part of trials
    valid = et_data_prepro[DataParser.DF_SAMPLES][mask]
    et_data_prepro[DataParser.DF_SAMPLES].loc[mask, f"{eye}StimDistDegs"] = valid[f"{eye}StimDistPixels"] * params['DegreesPerPix']  # convert to degrees
    et_data_prepro[DataParser.DF_SAMPLES].loc[mask, f"{eye}CenterDistDegs"] = valid[f"{eye}CenterDistPixels"] * params['DegreesPerPix']  # convert to degrees
    # We are also adding distances from CENTER separate to X coordinate and to Y coordinate (with direction)
    for coordi in ['X', 'Y']:
        center_coord = 0 if coordi == 'X' else 1
        et_data_prepro[DataParser.DF_SAMPLES].loc[mask, f"{eye}{coordi}CenterDistPixelsSigned"] = valid[f"{eye}{coordi}"] - center_coords[center_coord]  # in pixels
        et_data_prepro[DataParser.DF_SAMPLES].loc[mask, f"{eye}{coordi}CenterDistDegsSigned"] = et_data_prepro[DataParser.DF_SAMPLES].loc[mask, f"{eye}{coordi}CenterDistPixelsSigned"] * params['DegreesPerPix']  # in degrees
...
def analysis_windows(trial_info, et_data_dict, sub_params, is_tobii=False):
    ...
    TIME_WINDOWS = {"TimePostOnset": ("StimOnset", "EpochWindowEnd", "", 0),
                    "TimePost100": (DataParser.P100P600, f"{DataParser.P100P600}WindowEnd", "100", 100),
                    "TimePost200": (DataParser.P200P500, f"{DataParser.P200P500}WindowEnd", "200", 200),
                    "TimePost250": (DataParser.P250P500, f"{DataParser.P250P500}WindowEnd", "250", 250)}
    ...
    for window in [DataParser.STIM_DUR, DataParser.PRE_STIM_DUR, DataParser.TRIAL, DataParser.P100P600, DataParser.P200P500, DataParser.P250P500]:
        # The Current implementation IS WEIGHTED
        mean_fix = real_fix_samps.groupby([window]).mean().reset_index()
        mean_everything = samps.groupby([window]).mean().reset_index()
        for index, trial in mean_everything.iterrows():
            if trial[window] != -1:  # if -1, this means it is not part of the current window
                trial_info.loc[trial_info["trialNumber"] == trial[window], f"GazeCoords{window}IsInFixationArea"] = trial[f"{eye}IsInFixationArea"]

        for index, trial in mean_fix.iterrows():
            if trial[window] != -1:  # if -1, this means it is not part of the current window
                trial_info.loc[trial_info["trialNumber"] == trial[window], f"{window}StimDistDegs"] = trial[f"{eye}StimDistDegs"]
                trial_info.loc[trial_info["trialNumber"] == trial[window], f"X{window}StimDistDegs"] = trial[f"{eye}XStimDistDegsSigned"]
                trial_info.loc[trial_info["trialNumber"] == trial[window], f"Y{window}StimDistDegs"] = trial[f"{eye}YStimDistDegsSigned"]
                ...
                trial_info.loc[trial_info["trialNumber"] == trial[window], f"Fixation{window}IsInFixationArea"] = trial[f"{eye}IsInFixationArea"]

        median_fix = real_fix_samps.groupby([window]).median().reset_index()
        for index, trial in median_fix.iterrows():
            if trial[window] != -1:  # if -1, this means it is not part of the current window
                trial_info.loc[trial_info["trialNumber"] == trial[window], f"{window}{MEDIAN}StimDistDegs"] = trial[f"{eye}StimDistDegs"]
                trial_info.loc[trial_info["trialNumber"] == trial[window], f"X{window}{MEDIAN}StimDistDegs"] = trial[f"{eye}XStimDistDegsSigned"]
                trial_info.loc[trial_info["trialNumber"] == trial[window], f"Y{window}{MEDIAN}StimDistDegs"] = trial[f"{eye}YStimDistDegsSigned"]
    ...
    trial_info.loc[:, f"{DataParser.TRIAL}NumFix"] = 0  # nans to zeros, as these are blink counts
    real_fix_samps = real_fix_samps[real_fix_samps[DataParser.TRIAL] != -1]
    unique_fix = real_fix_samps.groupby([DataParser.TRIAL]).fix_number.nunique()
    for index, count in unique_fix.items():
        trial_info.loc[trial_info["trialNumber"] == index, f"{DataParser.TRIAL}NumFix"] = count
    ...
    if not is_tobii:  # no blinks in tobii eye data
        ...
        blink_dict_counter = {elem: {DataParser.STIM_DUR: 0, DataParser.PRE_STIM_DUR: 0, DataParser.TRIAL: 0} for elem in blink_numbers}
        for window in [DataParser.STIM_DUR, DataParser.PRE_STIM_DUR, DataParser.TRIAL]:
        ...
        for key in blink_dict_counter.keys():
            max = 0
            max_val = None
            for window in [DataParser.STIM_DUR, DataParser.PRE_STIM_DUR]:
        ...
        for window in [DataParser.STIM_DUR, DataParser.PRE_STIM_DUR, DataParser.TRIAL, DataParser.P100P600, DataParser.P200P500, DataParser.P250P500]:
            ...
            if window != DataParser.TRIAL:
                """
                Subjects might not have blinks in the chosen window, for example SD198
                """
                if samp_blinks[(samp_blinks[window] != -1)].empty:
                    continue
                if window == DataParser.P100P600 or window == DataParser.P250P500 or window == DataParser.P200P500:
                    samps_blink_window = samp_blinks[(samp_blinks[window] != -1) & (samp_blinks["MaxWindow"] != DataParser.PRE_STIM_DUR)]
                else:
                ...
                if samp_blinks[(samp_blinks[window] != -1)].empty:
                    continue
                samps_blink_window = samp_blinks[(samp_blinks[window] != -1)]  # all blink samples that are in that window
    ...
    for include_microsaccades in ["Micro", "", "both"]:
        only_micro = (include_microsaccades == "Micro")
        samp_saccs = samps[samps[DataParser.REAL_SACC] == True]  # take only samples that are real blinks (Hershman)
        if include_microsaccades != "both":
            samp_saccs = samp_saccs[samp_saccs["microsaccade"] == only_micro]
        sacc_numbers = list(samp_saccs["sacc_number"].unique())  # retrieve their blink numbers, as numbered in the parsing step
        ek_saccs = et_data_dict[DataParser.DF_SACC_EK]
        ek_saccs = ek_saccs[ek_saccs[DataParser.EYE] == eye]  # get rid of the non-analyzed eye
        ek_saccs = ek_saccs.loc[ek_saccs[f"{DataParser.HERSHMAN_PAD}"] == False, :].reset_index()
        # 29-01-26 important correction. Did not matter to analysis because we fixed it manually as well beforehand, but this is much more clear
        if include_microsaccades != "both":
            ek_saccs = ek_saccs[ek_saccs["microsaccade"] == only_micro]
        # initialize a dictionary where key=blink, val= a dictionary counting how many samples out of that blink are in each window
        sacc_dict_counter = {elem: {DataParser.STIM_DUR: 0, DataParser.PRE_STIM_DUR: 0, DataParser.TRIAL: 0}
        ...
        for key in sacc_dict_counter.keys():
            ...
            if max > 0:
                ...
                ek_saccs.loc[key-1, "MaxWindow"] = max_val # Saccade numbers start from 1, and index starts from 0

        for window in [DataParser.STIM_DUR, DataParser.PRE_STIM_DUR, DataParser.TRIAL, DataParser.P100P600, DataParser.P200P500, DataParser.P250P500]:
            if window != DataParser.TRIAL:
                """
                Some subjects (SC130), for example, do not have ANY saccade in the relevant window, hence check and continue if len is 0.
                """
                if samp_saccs[(samp_saccs[window] != -1)].empty:
                    continue

                if window == DataParser.P100P600 or window == DataParser.P250P500 or window == DataParser.P200P500:
                    samps_sacc_window = samp_saccs[(samp_saccs[window] != -1) & (samp_saccs["MaxWindow"] != DataParser.PRE_STIM_DUR)]  # all sacc samples that are in that window
                    ek_saccs_window = ek_saccs[(ek_saccs[window] != -1) & (ek_saccs["MaxWindow"] != DataParser.PRE_STIM_DUR)]
                else:
                    samps_sacc_window = samp_saccs[(samp_saccs[window] != -1) & (samp_saccs["MaxWindow"] == window)]  # all sacc samples that are in that window
                    ek_saccs_window = ek_saccs[(ek_saccs[window] != -1) & (ek_saccs["MaxWindow"] == window)]
            else:
                if samp_saccs[(samp_saccs[window] != -1)].empty:
                    continue
                samps_sacc_window = samp_saccs[(samp_saccs[window] != -1)]
                ek_saccs_window = ek_saccs[(ek_saccs[window] != -1)]

            # This average is ACROSS saccades, meaning a LONG saccade and a SHORT saccade contribute the same to the average
            mean_saccs_in_window = ek_saccs_window.groupby([window]).mean().reset_index()
            for index, max_sacc in mean_saccs_in_window.iterrows():
                trial_info.loc[trial_info["trialNumber"] == max_sacc[window], f"{window}{include_microsaccades}MeanAmp"] = max_sacc[f"ampDeg"]
                trial_info.loc[trial_info["trialNumber"] == max_sacc[window], f"{window}{include_microsaccades}sacc_direction_deg"] = max_sacc[f"sacc_direction_deg"]
                trial_info.loc[trial_info["trialNumber"] == max_sacc[window], f"{window}{include_microsaccades}sacc_direction_rad"] = max_sacc[f"sacc_direction_rad"]

            max_saccs_in_window = samps_sacc_window.groupby([window]).max().reset_index()
            for index, max_sacc in max_saccs_in_window.iterrows():
                trial_info.loc[trial_info["trialNumber"] == max_sacc[window], f"{window}{include_microsaccades}MaxAmp"] = max_sacc[f"ampDeg"]
            ...
            for index, count in unique_saccs_in_window.items():
                trial_info.loc[trial_info["trialNumber"] == index, f"{window}{include_microsaccades}NumSaccs"] = count

        for window2 in TIME_WINDOWS.keys():
            tmp_ek_sacc = ek_saccs.copy()
            # New Time Window for saccades
            found_any = False
            for index, trial in trial_info.iterrows():  # iterate trials
                window_start = trial[TIME_WINDOWS[window2][0]]
                window_end = trial[TIME_WINDOWS[window2][1]]
                #SD195 has NO EKSacc post onset, chekc if empty
                if tmp_ek_sacc.loc[tmp_ek_sacc[DataParser.T_START].between(window_start, window_end)].empty:
                    continue
                found_any = True
                tmp_ek_sacc.loc[tmp_ek_sacc[DataParser.T_START].between(window_start, window_end), "PostOnset"] = trial["trialNumber"]
                tmp_ek_sacc.loc[tmp_ek_sacc["PostOnset"] == trial["trialNumber"], "TimePostOnset"] = tmp_ek_sacc.loc[tmp_ek_sacc["PostOnset"] == trial["trialNumber"], DataParser.T_START] - window_start
                tmp_ek_sacc.loc[tmp_ek_sacc["PostOnset"] == trial["trialNumber"], "TimePostOnset"] = tmp_ek_sacc.loc[tmp_ek_sacc["PostOnset"] == trial["trialNumber"], "TimePostOnset"] + TIME_WINDOWS[window2][3]

            # SD195 has NO EKSacc post onset, use found_any
            if found_any:
                only_firsts = tmp_ek_sacc.groupby('PostOnset').first().reset_index()
                for index, saccs in only_firsts.iterrows():
                    trial_info.loc[trial_info["trialNumber"] == saccs["PostOnset"], f"First{include_microsaccades}SaccTimePostOnset{TIME_WINDOWS[window2][2]}"] = saccs[f"TimePostOnset"]
                    trial_info.loc[trial_info["trialNumber"] == saccs["PostOnset"], f"First{include_microsaccades}SaccAmpDeg{TIME_WINDOWS[window2][2]}"] = saccs[f"ampDeg"]

    for window in TIME_WINDOWS.keys():
        # New Time Window
        fixations = et_data_dict[DataParser.DF_FIXAT].copy()
        fixations = fixations[fixations[DataParser.EYE.lower()] == eye]  # get rid of the non-analyzed eye
        fixations = fixations.loc[fixations[f"{DataParser.HERSHMAN_PAD}"] == False, :].reset_index()
        fixations["fix_number"] = fixations.index + 1  # Fixation numbers start from 1, and index starts from 0

        start_str = TIME_WINDOWS[window][0]
        end_str = TIME_WINDOWS[window][1]
        for index, trial in trial_info.iterrows():  # iterate trials
            window_start = trial[start_str]
            window_end = trial[end_str]

            fixations.loc[fixations[DataParser.T_START].between(window_start, window_end), "PostOnset"] = trial["trialNumber"]
            fixations.loc[fixations["PostOnset"] == trial["trialNumber"], "TimePostOnset"] = fixations.loc[fixations["PostOnset"] == trial["trialNumber"], DataParser.T_START] - window_start
            fixations.loc[fixations["PostOnset"] == trial["trialNumber"], "TimePostOnset"] = fixations.loc[fixations["PostOnset"] == trial["trialNumber"], "TimePostOnset"] + TIME_WINDOWS[window][3]

        only_firsts = fixations.groupby('PostOnset').first().reset_index()
        for index, fix in only_firsts.iterrows():
            trial_info.loc[trial_info["trialNumber"] == fix["PostOnset"], f"FirstFix{window}"] = fix[f"TimePostOnset"]
            trial_info.loc[trial_info["trialNumber"] == fix["PostOnset"], f"FirstFixDur{TIME_WINDOWS[window][2]}"] = fix[f"duration"]

    return trial_info


def sacc_direction(et_data_prepro, params):
    """
    Calculate the direction of each (real) saccade in both pixels and degrees
    :param et_data_prepro:
    :return:
    """
    sacc = et_data_prepro[DataParser.DF_SACC]

    sacc[DataParser.SACC_DIRECTION_RAD] = np.nan
    sacc["sacc_direction_deg"] = np.nan

    mask = (pd.isna(sacc[f"is_{DataParser.EYELINK}Blink"]))
    valid = sacc[mask]
    delta_y = list(valid['yEnd'] - valid['yStart'])
    delta_x = list(valid['xEnd'] - valid['xStart'])
    sacc.loc[mask, "saccade_x"] = delta_x
    sacc.loc[mask, "saccade_y"] = delta_y

    dist_to_fix_onset = np.sqrt((valid['xStart'] - params['ScreenCenter'][0] * params['DegreesPerPix']) ** 2 +
                                (valid['yStart'] - params['ScreenCenter'][1] * params['DegreesPerPix']) ** 2)
    dist_to_fix_offset = np.sqrt((valid['xEnd'] - params['ScreenCenter'][0] * params['DegreesPerPix']) ** 2 +
                                 (valid['yEnd'] - params['ScreenCenter'][1] * params['DegreesPerPix']) ** 2)
    distToFix = (dist_to_fix_offset - dist_to_fix_onset)
    sacc.loc[mask, "saccade_dist_to_center"] = distToFix

    sacc.loc[mask, DataParser.SACC_DIRECTION_RAD] = [math.atan2(delta_y[i], delta_x[i]) for i in range(len(delta_x))]
    sacc.loc[mask, "sacc_direction_deg"] = sacc.loc[mask, DataParser.SACC_DIRECTION_RAD] * 180 / math.pi
    """
...
def sacc_amp(et_data_prepro, params):
    """
    Calculate the distance of the saccade (r in polar coordinates (r, theta)): in PIXELS
    NOTE: THIS IS *COMPLETELY* EQUIVALENT to the "ampDeg" field, which is directly outputted from EyeLink and
    reflects the saccade's amplitude. i.e, in trial sacc, "ampDeg" column and "sacc_dist" column are the SAME data
    but with ampDeg being in visual angle degrees (according to EyeLink guide) and sacc_dist in Pixels.
    :param et_data_prepro:
    :return:
    """
    sacc = et_data_prepro[DataParser.DF_SACC]

    sacc["sacc_dist_dva"] = np.nan

    mask = (pd.isna(sacc[f"is_{DataParser.EYELINK}Blink"]))
    valid = sacc[mask]
    delta_y = list((valid['yEnd'] - valid['yStart']) * params['DegreesPerPix'])
    delta_x = list((valid['xEnd'] - valid['xStart']) * params['DegreesPerPix'])
    sacc.loc[mask, "sacc_dist_dva"] = [np.sqrt(delta_x[i]**2 + delta_y[i]**2) for i in range(len(delta_x))]
...
def blink_detection(et_data, params):
    ...
    num_of_blink_samples = blink_data["blink_onset"].shape[0] if isinstance(blink_data['blink_onset'],
                                                                            np.ndarray) else 0
    """
    ...
    if num_of_blink_samples > 0:
        ...
        for i in range(num_of_blink_samples):
            ...
            et_samples.loc[blink_start: blink_end,
            f"{eye}{DataParser.HERSHMAN}"] = 1  # the range here is up to and including, i.e., 1:3 is 1, 2, 3
...
def blink_detection_new(et_data, params):
    """
    Identify real blinks from all samples.
    NOTE :: After running this, we might get a sample that's both a non-blink (e.g., fixation/saccade) AND a "Hershman"
    blink. Hershman blinks are considered the ground truth. Thus, any blink that is not Hershman is removed from dfBlink.
    ...
    blink_counter = 1
    for file_num in list(et_samples["file"].unique()):
        et_samps_file = et_samples.loc[et_samples["file"] == file_num, :]
        first_item_ind = et_samps_file.index[0]
        convert_for_hershman = np.array(et_samps_file[col], dtype="float32")  # as per their OSF example: https://osf.io/gjt8v/
        blink_data = based_noise_blinks_detection(pupil_size=convert_for_hershman, sampling_freq=sampling_freq)
        ...
        if num_of_blink_samples > 0:
            # add a column with the duration in MILLISECONDS of each blink (divide in freq for SECONDS, *1000 for MILLISECONDS)
            blink_data['duration'] = 1000 * (blink_data['blink_offset'] - blink_data['blink_onset']) / sampling_freq
            blink_data['tStart'] = list(et_samples.loc[blink_data['blink_onset'] + first_item_ind, "tSample"])
            blink_data['tEnd'] = list(et_samples.loc[blink_data['blink_offset'] + first_item_ind, "tSample"])
            blink_data['eye'] = [eye] * len(blink_data['tEnd'])
            blink_data['file'] = [file_num] * len(blink_data['tEnd'])
            blink_data[DataParser.HERSHMAN] = [True] * len(blink_data['tEnd'])
            blink_df = pd.DataFrame.from_dict(blink_data)[['eye', 'tStart', 'tEnd', 'duration', DataParser.HERSHMAN, 'file']]
            et_blinks = pd.concat([et_blinks, blink_df])
            for i in range(num_of_blink_samples):
                blink_start = int(blink_data["blink_onset"][i])
                blink_end = int(blink_data["blink_offset"][i])
                et_samples.loc[blink_start + first_item_ind: blink_end + first_item_ind, f"{eye}{DataParser.HERSHMAN}"] = 1  # the range here is up to and including, i.e., 1:3 is 1, 2, 3
                et_samples.loc[blink_start + first_item_ind: blink_end + first_item_ind, "blink_number"] = blink_counter
                blink_counter += 1
    ...
    """
    et_blinks = et_blinks[et_blinks[f"{DataParser.HERSHMAN}"] == 1]
    et_blinks.reset_index(drop=True, inplace=True)
    firsts = et_samples.groupby(["file"]).first()
    lasts = et_samples.groupby(["file"]).last()
    et_blinks = et_blinks.loc[~((et_blinks["tStart"].isin(firsts["tSample"])) | (et_blinks["tEnd"].isin(lasts["tSample"]))), :]
    et_data[DataParser.DF_BLINK] = et_blinks
    return et_data
...
def extract_microsaccades(eye_gaze, velocity, speed, params):
    ...
    for i in range(n_sacs):
        ...
        28-01-24 UPDATE:
        Seems like the configuration flips it as well, so we dont flip it.
        """
...
def saccade_detection(et_data_dict, params):
    ...
    *** NOTE *** :
    ...
    for eye in EYES:
        ...
        """ parallel to experiment 1's ET "GetVelocity" 
        ...
        # data samples
        for n in range(2, eye_gaze.shape[0] - 2):
            velocity[n, :] = (eye_gaze[n + 1, :] + eye_gaze[n + 2, :] - eye_gaze[n - 1, :] - eye_gaze[n - 2, :]) * (
                        params["SamplingFrequency"] / 6)
    ...
    """
    Calculate binocular saccades by looking for overlap between two monocular saccades
    ...
    """
    In the Hershman blink detection, we could run the detection algorithm per file.
    With the EK algorithm, however, the detection is based on deviation from mean, hence we want to do it 
    for ALL of the subjects samples, and not per file. 
    Because of this, we need to remove saccades that overlap between two files, as they are incorrect.
    """
    EK_sacc_df = EK_sacc_df.loc[EK_sacc_df["duration"] == EK_sacc_df[DataParser.T_END] - EK_sacc_df[DataParser.T_START], :]
    et_data_dict[DataParser.DF_SACC_EK] = EK_sacc_df

    return et_data_dict


def unify_saccades(et_data_dict):
    """
    This function takes all the MICROsaccades based on E&K algorithm output, and all the saccades from Eyelink, and puts
    them together in a unified df that includes all saccade information. Notably, only columns that are shared among
    the two data types are in the new unified df. This REPLACES "dfSaccEK" value in the et_data_dict dicionary, and
    will be used for saccade analysis.
    """
    shared_columns = [DataParser.EYE, DataParser.T_START, DataParser.T_END, "duration", DataParser.AMP_DEG, "saccade_x",
                      "saccade_y", "saccade_dist_to_center", "sacc_dist_dva", "sacc_direction_deg", "sacc_direction_rad", "microsaccade"]
    micro_saccades = et_data_dict[DataParser.DF_SACC_EK][et_data_dict[DataParser.DF_SACC_EK]["microsaccade"] == True]
    micro_saccades = micro_saccades[shared_columns]
    saccades = et_data_dict[DataParser.DF_SACC]
    saccades[DataParser.EYE] = saccades[DataParser.EYE.lower()]
    saccades["microsaccade"] = False
    saccades = saccades[shared_columns]
    unified_df = pd.concat([saccades, micro_saccades])
    unified_df.sort_values(by=DataParser.T_START, inplace=True)
    unified_df.reset_index(drop=True, inplace=True)
    et_data_dict[DataParser.DF_SACC_EK] = unified_df
...
def mark_saccades(et_data_dict, params):
    ...
    for sacc in ek_saccs.itertuples():
        samps.loc[samps[DataParser.T_SAMPLE].between(sacc.tStart, sacc.tEnd), f"{DataParser.EK}Sacc"] = 1
        samps.loc[samps[DataParser.T_SAMPLE].between(sacc.tStart, sacc.tEnd), f"{DataParser.EK}{DataParser.AMP_DEG}"] = sacc.ampDeg  # amplitude
    ...
    for index, blink in et_data_dict[DataParser.DF_BLINK].iterrows():
        # Labeling an SSACC...ESACC pair with one or more SBLINK events between them as BLINKS
        ek_saccs.loc[(ek_saccs[DataParser.T_START] <= blink[DataParser.T_START]) & (ek_saccs[DataParser.T_END] >= blink[DataParser.T_START]) & (
                ek_saccs[DataParser.EYE] == blink['eye']), f"is_{DataParser.EYELINK}Blink"] = 1

    print("After second loop")

    bls = et_data_dict[DataParser.DF_BLINK]
    bls = bls[bls[DataParser.HERSHMAN] == True]
    for index, blink in bls.iterrows():
        # Labeling an SSACC...ESACC pair with one or more SBLINK events between them as BLINKS
        ek_saccs.loc[(ek_saccs[DataParser.T_START] <= blink[DataParser.T_START]) & (ek_saccs[DataParser.T_END] >= blink[DataParser.T_START]) & (
                ek_saccs[DataParser.EYE] == blink['eye']), f"is_{DataParser.HERSHMAN}Blink"] = 1

    print("After third loop")

    et_data_dict[DataParser.DF_SACC_EK] = ek_saccs
    return et_data_dict

def mark_saccades_new(et_data_dict, params):
    """
...
def mark_fixations(et_data_dict, params):
    ...
    fixations.loc[:, f"{DataParser.REAL_SACC}"] = False
    for index, blink in et_data_dict[DataParser.DF_BLINK].iterrows():
        """
    ...
    """
    Fixation is only a fixation if it does not overlap with a REAL SACCADE (not microssacade)
    """
    saccades = et_data_dict[DataParser.DF_SACC_EK]
    saccades = saccades[(saccades[DataParser.EYE] == eye) & (saccades[f"{DataParser.HERSHMAN_PAD}"] == False) & (saccades["microsaccade"] == False)]

    for index, sacc in saccades.iterrows():
        """
        ONE WAY: a fixation is nullified if it started during a saccade period, OR ENDED during a saccade period. 
        """
        fixations.loc[(((fixations[DataParser.T_START] >= sacc[DataParser.T_START]) & (fixations[DataParser.T_START] <= sacc[DataParser.T_END])) |
                      ((fixations[DataParser.T_END] >= sacc[DataParser.T_START]) & (fixations[DataParser.T_END] <= sacc[DataParser.T_END]))) & (
                      fixations[DataParser.EYE.lower()] == sacc[DataParser.EYE]), f"{DataParser.REAL_SACC}"] = True

        """
        SECOND WAY: a fixation is nullfied if it contains a whole saccade period inside it
        """
        fixations.loc[(((fixations[DataParser.T_START] <= sacc[DataParser.T_START]) & (fixations[DataParser.T_END] >= sacc[DataParser.T_START])) |
                      ((fixations[DataParser.T_START] <= sacc[DataParser.T_END]) & (fixations[DataParser.T_END] >= sacc[DataParser.T_END]))) & (
                      fixations[DataParser.EYE.lower()] == sacc[DataParser.EYE]), f"{DataParser.REAL_SACC}"] = True

    print(f"Mark fixations {datetime.datetime.now()}")
    fixations = fixations[fixations[DataParser.EYE.lower()] == eye]  # get rid of the non-analyzed eye
    samps.loc[:, DataParser.REAL_FIX] = False
    fix_no_blink = fixations.loc[(fixations[f"{DataParser.HERSHMAN_PAD}"] == False) & (fixations[f"{DataParser.REAL_SACC}"] == False), :]
    ...
    for fix in fix_no_blink.itertuples():
        ...
        samps.loc[samps[DataParser.T_SAMPLE].between(fix.tStart, fix.tEnd), "fix_number"] = fix_counter
        fix_counter += 1
...
def preprocess_beh_data(sub_beh_data):
    """
    Unify the GAME and REPLAY probe data (without practice levels in game) to a single df such that each row=trial.
    As replay data we take the CORRECTED probe data info: for more information see the behavioral analysis.
    :param sub_beh_data:
    :return:
    """
    game_data = sub_beh_data.full.SessDetails.ProbeDet
    game_data = game_data[game_data["activeWorldID"] != -1]  # practice levels in game
    replay_data = sub_beh_data.full.SessDetails.LocalizerDetFullCorrected
    replay_data = replay_data[~replay_data["stimulusID"].isna()]  # get rid of FILLERS

    # to unify the dataframes, we need to add some columns to game_data and remove some from replay_data
    replay_data = replay_data.drop(["duringSafeEndOfGame"], axis=1)
    replay_data.loc[:, DataParser.REPLAY] = True
    game_data.loc[:, DataParser.IS_LEVEL_ELIMINATED] = 0  # as no game levels are eliminated during the analysis
    game_data.loc[:, "TargetType"] = game_data.loc[:, "stimulusType"]  # as in the game there is no "target" type to focus on
    game_data.loc[:, DataParser.REPLAY] = False
    unified = pd.concat([game_data, replay_data]).reset_index(drop=True, inplace=False)

    unprobed_data = sub_beh_data.full.SessDetails.StimulusDet
    unprobed_data = unprobed_data[unprobed_data["activeWorldID"] != -1]  # practice levels in game
    unprobed_data = unprobed_data[unprobed_data["type"] == "GAME_STIMULUS"]
    unprobed_data.loc[:, DataParser.IS_LEVEL_ELIMINATED] = 0  # as no game levels are eliminated during the analysis

    return unified, unprobed_data


def extract_data(sub_code, session, et_path, asc_files, save_path, session_info_path, sub_beh_data):  # example_log_path
    """
    ...
    :param session: session number of the relevant experimental session the ET files should come from.
    :param et_path: the path to subject's "ET" folder, which assumed to be organized in accordance with the XNAT
    convention.
    :param beh_dir: the path to the subject's "BEH" folder, which assumed to be organized in accordance with the XNAT
    convention. Uses JUST for session info details.
    :param beh_data_path: the path to where the Subject structs are - where to load pickle files of the subjects' trial
    behavior from
    :param asc_files: path to the the folder of the relevant ascii files (ET raw data).
    :param example_log_path: path to an examplar behvaioral log file, to extract data about the subject's trials.
    ...
    params = ET_param_manager.InitParams(sub_code, session_info_path, ascii_file_path)

    # if data was already extracted
    pick_file = [f for f in os.listdir(save_path) if fnmatch.fnmatch(f, '%sEyeTrackingDataNew30-01-26.pickle' % sub_code)]

    subs_to_skip = ["SC151", "SC174"]
    if sub_code in subs_to_skip:
        return

    if len(pick_file) > 0:
        print(f"Subject {sub_code} already has a pickle file, QC will use the existing file and not re-extract data")
        modify_time = os.path.getmtime(os.path.join(save_path, pick_file[0]))
        print(modify_time)
        modify_date = datetime.datetime.fromtimestamp(modify_time)
        aug23 = datetime.datetime(2026, 1, 26, 0, 0)

        if modify_date > aug23 or "SC151" in sub_code:
            print(f"Subject {sub_code} already has a newer file, do nothing")
            return
    ...
    """
    et_data = dict.fromkeys([DataParser.DF_REC, DataParser.DF_MSG, DataParser.DF_FIXAT, DataParser.DF_SACC, DataParser.DF_BLINK, DataParser.DF_SAMPLES])
    ...
    file_ind = 0
    for fl in asc_files:
        if fl.endswith("RestinEO.asc"):  # skip this file - this is rest ET data for MEEG
            continue
        if fl.endswith("_INTERRUPTED"):  # skip duplications: an interrupted file which also has an uninterrupted version
            no_inter = fl.replace("_INTERRUPTED", "")
            if no_inter in asc_files:
                continue
        worldNo = fl.split('.')[0][-2]  # get the world number
        if worldNo != 'A' and worldNo != 'B' and worldNo != 'E':  # GAME world is numeric, replay not - "E" is a PKU fluke
            worldNo = int(worldNo)
        print(f'-- World {worldNo} --')
        ...
        flEyeData[DataParser.DF_MSG]['WorldID'] = [f"World {worldNo}"] * flEyeData[DataParser.DF_MSG].shape[0]
        flEyeData[DataParser.DF_MSG]['WorldName'] = np.array([sub_code + '_' + str(worldNo)] * flEyeData[DataParser.DF_MSG].shape[0])
        flEyeData[DataParser.DF_SAMPLES]["file"] = file_ind
        file_ind += 1
        # now, add this file's parsed information to the general dictionary (et_data) that contains all files' data
        for key in [DataParser.DF_REC, DataParser.DF_MSG, DataParser.DF_FIXAT, DataParser.DF_SACC, DataParser.DF_BLINK, DataParser.DF_SAMPLES]:
    ...
    for key in [DataParser.DF_REC, DataParser.DF_MSG, DataParser.DF_FIXAT, DataParser.DF_SACC, DataParser.DF_BLINK, DataParser.DF_SAMPLES]:
        if et_data[key].empty:  # if the dataframe has nothing in it after parsing ALL files
            print(f"Issue with subject's {sub_code} session {session}: no {key} at all in the entire session. Inspect manually")
            return

    params = ET_param_manager.UpdateParams(params, et_data[DataParser.DF_MSG])

    """
    At this point we have in et_data information for the subject's entire experiment about:
    ET samples, blinks, fixations and saccades. Now we will create a dictionary named et_data_prepro
    # which is the PREPROCESSED ET data :
    - blinks dataframe is used to clean out artificial fixations/saccades. Remember, according to Eyelink blinks are
    times where "the pupil size is very small, or the pupil in the camera image is missing or severely distorted by eyelid occlusion".
    - fixations and saccades dataframes have an additional "is_blink" column that tags artificial data, meaning not 
    real fixation / saccade, based on the method we chose to preprocess
    - samples dataframe contains additional columns to tag each sample as blink/fixation/saccade (real)
    """

    """TIMINGS, ALIGNMENT AND REPORT"""
    timings_offset = DataParser.check_timings(et_data)  # THIS ONLY **REPORTS** THE OFFSET IN SAMPLES AS CALCULATED IN THE METHOD
    timings_offset.to_csv(os.path.join(save_path, f"{sub_code}_timings_report_{int(params['SamplingFrequency'])}.csv"), index=False)
    ...
    pick_path = os.path.join(save_path, f"{sub_code}_et_data_eyelink.pickle")
    et_data_eyelink = DataParser.et_data_mark_Eyelink(et_data)
    """
    DEPRECATED
    fl = open(pick_path, 'wb')
    pickle.dump(et_data_eyelink, fl)
    fl.close()
    """
    ...
    et_data_hershman = blink_detection_new(et_data_eyelink, params)

    # Pad blinks with et_param_manager.BLINK_PAD_MS window before and after. This is used for saccade/fixation/pupil filtering
    et_data_hershman = pad_blinks(et_data_hershman, params)

    print("\n--- Data Pre-Processing: Saccades ---")
    print(f"Running E&K's saccade and microsaccade detection algorithm {datetime.datetime.now()}")
    et_data_saccades = saccade_detection(et_data_hershman, params)

    print("Unify EK microsaccades with Eyelink saccades")
    # Deprecated, we ignore saccades which are not EK
    #print("Calculating saccade direction information")
    #et_data_hershman = sacc_direction(et_data_hershman, params)
    #print("Calculating saccade amplitude information from Eyelink")
    #et_data_hershman = sacc_amp(et_data_hershman, params)
    #et_data_saccades = unify_saccades(et_data_saccades)
    #et_data_saccades = mark_saccades(et_data_saccades, params)

    print(f"mark samples with EK data")
    et_data_saccades = mark_saccades_new(et_data_saccades, params)

    print(f"mark samples with fixation data")
    et_data_saccades = mark_fixations(et_data_saccades, params)  # this marks Eyelink fixations IFF they don't overlap with padded blink periods
    ...
    """
    et_data_saccades = mark_pupils(et_data_saccades, params)  # this nullifies pupil size IFF it overlaps with padded blink periods

    print('Getting behavioral trial information (VG stimuli and responses)')
    sess_beh_data, unprobed_sess_beh_data = preprocess_beh_data(sub_beh_data)
    ...
    et_trial_info, et_trial_info_unprobed = DataParser.get_trial_info(et_data[DataParser.DF_MSG])
    ...
    trial_info = DataParser.set_trial_info(et_trial_info, sess_beh_data, sub_code, save_path)
    trial_info_unprobed = DataParser.set_trial_info(et_trial_info_unprobed, unprobed_sess_beh_data, sub_code, save_path, is_probed=False)

    # Unify trial info and unprobed trial info
    trial_info.loc[:, IS_PROBED] = True
    trial_info_unprobed.loc[:, IS_PROBED] = False
    trial_info_unprobed.loc[:, DataParser.REPLAY] = False
    trial_info = pd.concat([trial_info, trial_info_unprobed])
    # Sub SD-120 has trials without ONSET. REMOVE THEM.
    if sub_code == "SD120":
        trial_info = trial_info.dropna(subset=[ONSET])
    trial_info.sort_values(by=[data_reader.STIM_ONSET_TS], inplace=True)
    trial_info.reset_index(drop=True, inplace=True)
    trial_info.loc[:, "trialNumber"] = trial_info.index

    print('Sequencing trials')
    et_data_fixations, trial_info = DataParser.et_data_to_trials(et_data_saccades, trial_info, params)
    ...
    # and whether or not the distance from fixation is counted as gazing at the fixation area (as per the parameter that
    # defines the radius within which we consider a gaze to be "within the area of fixation").
    et_data_fixations = dist_from_target(trial_info, et_data_fixations, params)
    ...
    """
    trial_info = analysis_windows(trial_info, et_data_fixations, params)
    trial_info.to_csv(os.path.join(save_path, f"{sub_code}_TrialInfoNew30-01-26.csv"), index=False)

    # pickle the data
    print("\n--- Done Data Pre-Processing ---")
    print('Saving subject eye tracking data...')
    fl = open(os.path.join(save_path, f"{sub_code}EyeTrackingDataNew30-01-26.pickle"), 'wb')
    sub_data = {"trial_info": trial_info, "et_data_dict": et_data_fixations, "params": params}
