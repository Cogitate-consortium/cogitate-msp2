...
def create_derivative_qc_hpc(modality, root_folder=data_reader.COGITATE_PATH):
...
def create_sub_qcs_hpc(sub_name, modality, root_folder=data_reader.COGITATE_PATH):
    ...
    ses_folder = os.path.join(sub_folder, f"{SESS_V2}")
...
def create_hpc_quality_checks(root_folder):
    qc_dir = os.path.join(root_folder, HPC_QC_FOLDER, V2, BEH)
    if not os.path.isdir(qc_dir):
        create_dir(qc_dir)
    return qc_dir
