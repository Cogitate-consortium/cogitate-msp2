...
def get_trans_from_mri(fname_raw, task):
    ''' This code loads the trans file from the MRI.
    It can be used with the pubblic Cogitate database. Does not work on the private XNAT database. '''
    import numpy as np
    from mne import head_to_mri
    from mne_bids import get_head_mri_trans, BIDSPath
    
    raw = mne.io.read_raw_fif(fname_raw, allow_maxshield=True)
    pos = np.asarray((raw.info["dig"][0]["r"], raw.info["dig"][1]["r"], raw.info["dig"][2]["r"]))
    bids_path = BIDSPath(subject=subject[4:], session=visit, task=task, run="01", root=opt.bids_root)
    estim_trans = get_head_mri_trans(bids_path=bids_path, fs_subject=subject, fs_subjects_dir=subjects_dir)
    trans = head_to_mri(pos=pos, subject="sample", mri_head_t=estim_trans, subjects_dir=subjects_dir)
...
def make_forward_model(src, task):
    ...
    try:
        ...
        trans = op.join(fname_coreg, subject.replace("-", "_")+"_ses-"+visit+"_trans.fif")
        bem = op.join(subjects_dir, subject, subject+"_ses-V1_bem-sol.fif") #BEM is shared between sessions
