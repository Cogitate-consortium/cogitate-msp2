...
def Seen_WCD(epochs_rs,et_data,
                 #select_F,
                 #n_trials,
 #                nPCA,
    ...
    temp[epochs_rs.metadata['Response'] == condition_Trial[0]] = 1  # Seen
    temp[epochs_rs.metadata['Response'] == condition_Trial[1]] = 2 # Unseen
    ...
    wcd['seen']=np.mean(subsamp_scores, axis=0)       
    ...
    ax.set_title(f'WCD_seen_ {ch_index}')
...
if __name__ == "__main__":
    ...
    for ch in range(2):
        fname_fig_acc = op.join(dAT_figure_root, 
                            sub_info  
                            + "_acc_dAT_seen_" +ch_index[ch] + '.png')
        c_index='Seen_'+ch_index[ch]
        temp_acc = Seen_WCD(epochs,ET_4ch,ch_index[ch],score_methods,fname_fig_acc)
