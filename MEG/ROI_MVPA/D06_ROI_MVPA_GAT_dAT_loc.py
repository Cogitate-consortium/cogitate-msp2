...
def Category_CTWCD(epochs_rs,stcs,
 ...
 #                nPCA,
    ...
    subsamp_scores=np.zeros([100,X.shape[2],X.shape[2]])
    ...
    for condi in range(2):
        ...
        ctwcd[condition_Trial[condi]]=np.mean(subsamp_scores, axis=0)       
    ...
    #plot
    im = axes[0].imshow(gaussian_filter(ctwcd[condition_Trial[0]],sigma=2), interpolation='lanczos', origin='lower', cmap=cmap,
    ...
    im = axes[1].imshow(gaussian_filter(ctwcd[condition_Trial[1]], sigma=2), interpolation='lanczos', origin='lower', cmap=cmap,
                   extent=epochs_rs.times[[0, -1, 0, -1]], vmin=vmin, vmax=vmax)
...
if __name__ == "__main__":
    ...
    analysis_name='GAT_PFC_dAT_loc_full'
    task_info='vg'
    ...
    for nroi, roi_name in enumerate(ROI_Name):
        ...
        fname_fig_acc = op.join(roi_figure_root, 
                            sub_info  + 'Loc' + '_' + roi_name + "_acc_CTWCD" + '.png')
    ...
    fname_data=op.join(roi_data_root, sub_info + '_'  +"Loc_ROIs_data" + '.pickle')
