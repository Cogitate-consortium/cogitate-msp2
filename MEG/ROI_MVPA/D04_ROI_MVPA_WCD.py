...
def Category_WCD(epochs_rs1,stcs1,epochs_rs2,stcs2,index_name,conditions_C,
                 select_F,
                 n_trials,
                 #nPCA,
    ...
    for num_per in range(100):
        ...
        #subsamp_scores[num_per,:]=np.mean(scores, axis=0) 
                
            
    wcd[con_index]=np.mean(scores_per, axis=0)       
...
if __name__ == "__main__":
    ...
    for nroi, roi_name in enumerate(ROI_Name):
        ...
        wcd_acc_cat= Category_WCD(epochs_rs1,stcs1,epochs_rs2,stcs2,
                              index_name,conditions_C,
                              select_F,n_trials,roi_name,
                              score_methods,fname_fig_cat)

        roi_wcd_cat[roi_name] = wcd_acc_cat
        ...
        fname_fig_loc = op.join(roi_figure_root, 
                            sub_info  + 'Loc' + '_' + roi_name + "_acc_WCD" + '.png')
        
        wcd_acc_loc= Category_WCD(epochs_rs1,stcs1,epochs_rs2,stcs2,
                              index_name,conditions_C,
                              select_F,n_trials,roi_name,
                              score_methods,fname_fig_loc)

        roi_wcd_loc[roi_name] = wcd_acc_loc
    ...
    roi_data['wcd_acc_cat']=roi_wcd_cat
    roi_data['wcd_acc_loc']=roi_wcd_loc
    
   



    fname_data=op.join(roi_data_root, sub_info + '_'  +"WCD_ROIs_data" + '.pickle')
