...
from D_MEG_function_E2 import source_data_for_ROI_MVPA,sub_ROI_for_ROI_MVPA
...
def Category_CCD(epochs_rs1,stcs1,epochs_rs2,stcs2,index_name,conditions_C,select_F,n_trials,roi_name,score_methods,fname_fig):
    ...
    ccd['A2B'] = np.mean(scores_ab_per, axis=0)
    ccd['B2A'] = np.mean(scores_ba_per, axis=0)
...
if __name__ == "__main__":
    ...
    for nroi, roi_name in enumerate(ROI_Name):
        ...
        ccd_acc_cat= Category_CCD(epochs_rs1,stcs1,epochs_rs2,stcs2,
                              index_name,conditions_C,
                              select_F,n_trials,roi_name,
                              score_methods,fname_fig_cat)

        roi_ccd_cat[roi_name] = ccd_acc_cat
        ...
        ccd_acc_loc= Category_CCD(epochs_rs1,stcs1,epochs_rs2,stcs2,
                              index_name,conditions_C,
                              select_F,n_trials,roi_name,
                              score_methods,fname_fig_loc)

        roi_ccd_loc[roi_name] = ccd_acc_loc
    ...
    roi_data['ccd_acc_cat']=roi_ccd_cat
    roi_data['ccd_acc_loc']=roi_ccd_loc
    


    fname_data=op.join(roi_data_root, sub_info + '_'  +"CCD_ROIs_data" + '.pickle')
