...
def set_path_AT_MVPA(bids_root,subject_id, visit_id, analysis_name):
...
def Loc_AT(epochs,score_methods,fname_fig_wcd,fname_fig_ctwcd):
    ...
    #condition_Trial=['Target','Non-Target']
    condition_Trial=['AT_combine']
    condition_Stim=['Upper Right','Lower Right','Upper Left','Lower Left']
    

    temp = epochT.events[:, 2]
    temp[epochT.metadata['Location'] == condition_Stim[0]] = 1  # right
    temp[epochT.metadata['Location'] == condition_Stim[1]] = 1 # right
    temp[epochT.metadata['Location'] == condition_Stim[2]] = 2  # left
    temp[epochT.metadata['Location'] == condition_Stim[3]] = 2 # left
    ...
    AT_WCD=dict()
    AT_CTWCD=dict()
    for condi in range(1):
        con_index_T=np.where(((epochT.metadata['Trial_type'] == 'Target') & (epochT.metadata['Response']=='seen')))[0]
        con_index_NT=np.where(((epochT.metadata['Trial_type'] == 'Non-Target')& (epochT.metadata['Response']!='seen')))[0]
        con_index=np.append(con_index_T,con_index_NT)
        group_x=X[con_index]
        group_y=y[con_index]
        ...
        AT_WCD[condition_Trial[condi]]=np.mean(scores_per_wcd, axis=0)
        AT_CTWCD[condition_Trial[condi]]=np.mean(scores_per_ctwcd, axis=0)
    ...
    #plot
    im = ax.imshow(gaussian_filter(AT_CTWCD[condition_Trial[0]],sigma=2), interpolation='lanczos', origin='lower', cmap=cmap,
                   extent=epochT.times[[0, -1, 0, -1]], vmin=vmin, vmax=vmax)
    ax.set_xlabel('Testing Time (s)')
    ax.set_ylabel('Training Time (s)')
    ax.set_title('AT_GAT_loc')
    ax.axvline(0, color='k')
    ax.axhline(0, color='k')
    ax.axline((0, 0), slope=1, color='k')
    plt.colorbar(im, ax=ax,fraction=0.03, pad=0.05)

    # im = axes[1].imshow(gaussian_filter(AT_CTWCD[condition_Trial[1]], sigma=2), interpolation='lanczos', origin='lower', cmap=cmap,
    #                extent=epochT.times[[0, -1, 0, -1]], vmin=vmin, vmax=vmax)
...
if __name__ == "__main__":
    ...
    analysis_name='Sensor_AT_loc'

    # 1 Set Path
    sub_info, \
    fpath_epo, fpath_fw, fpath_fs, \
    AT_data_root, AT_figure_root, AT_code_root = set_path_AT_MVPA(bids_root,
                                                                      subject_id,
                                                                      visit_id,
                                                                      analysis_name)

    fname_fig_wcd = op.join(AT_figure_root, 
                            sub_info  
                            + "_acc_AT_loc_wcd" + '.png')
    
    fname_fig_ctwcd = op.join(AT_figure_root, 
                            sub_info  
                            + "_acc_AT_loc_ctwcd" + '.png')
    ...
    AT_wcd,AT_ctwcd= Loc_AT(epochs,score_methods,fname_fig_wcd,fname_fig_ctwcd)
    ...
    sensor_AT['wcd']=AT_wcd
    sensor_AT['ctwcd']=AT_ctwcd


    fname_data=op.join(AT_data_root, sub_info + '_' + task +"_AT_loc_acc" + '.pickle')
    fw = open(fname_data,'wb')
    pickle.dump(sensor_AT,fw)
    fw.close()
