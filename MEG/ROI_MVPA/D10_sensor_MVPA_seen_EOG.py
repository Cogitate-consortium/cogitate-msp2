...
def Seen_dAT(epochs,score_methods,fname_fig_wcd,fname_fig_ctwcd):
    ...
    temp[epochT.metadata['Response'] == condition_Trial[0]] = 1  # Seen
    temp[epochT.metadata['Response'] == condition_Trial[1]] = 2 # Unseen

    y = temp
    X=epochT.pick_types(eog=True).get_data()
    ...
    dAT_CTWCD=dict()
        ...
        #subsampleing the data
        
    
    
    con_index_seen=np.where(epochT.metadata['Response'] == condition_Trial[0])[0]
    con_index_unseen=np.where(epochT.metadata['Response'] == condition_Trial[1])[0]
    ...
    scores_per_wcd=np.zeros([100,X.shape[2]])
    scores_per_ctwcd=np.zeros([100,X.shape[2],X.shape[2]])
    for num_per in range(100):
        ...
        group_x_seen=X[con_index_seen[:Trial_min]]
        group_x_unseen=X[con_index_unseen[:Trial_min]]
        
        group_y_seen=y[con_index_seen[:Trial_min]]
        group_y_unseen=y[con_index_unseen[:Trial_min]]
        
        group_x=np.append(group_x_seen,group_x_unseen,axis=0)
        group_y=np.append(group_y_seen,group_y_unseen,axis=0)
    ...
    dAT_WCD['seen']=np.mean(scores_per_wcd, axis=0)
    dAT_CTWCD['seen']=np.mean(scores_per_ctwcd, axis=0)
    ...
    #plot
    im = ax.imshow(gaussian_filter(dAT_CTWCD['seen'],sigma=2), interpolation='lanczos', origin='lower', cmap=cmap,
                   extent=epochT.times[[0, -1, 0, -1]], vmin=vmin, vmax=vmax)
    ax.set_xlabel('Testing Time (s)')
    ax.set_ylabel('Training Time (s)')
    ax.set_title('Seen_decoding')
    ax.axvline(0, color='k')
...
if __name__ == "__main__":
    ...
    analysis_name='Sensor_dAT_seen_EOG'
    ...
    fname_fig_wcd = op.join(dAT_figure_root, 
                            sub_info  
                            + "_acc_dAT_seen_wcd" + '.png')
    
    fname_fig_ctwcd = op.join(dAT_figure_root, 
                            sub_info  
                            + "_acc_dAT_seen_ctwcd" + '.png')
    ...
    dAT_wcd,dAT_ctwcd= Seen_dAT(epochs,score_methods,fname_fig_wcd,fname_fig_ctwcd)
    ...
    sensor_seen['wcd']=dAT_wcd
    sensor_seen['ctwcd']=dAT_ctwcd


    fname_data=op.join(dAT_data_root, sub_info + '_' + task +"_dAT_seen_acc" + '.pickle')
    fw = open(fname_data,'wb')
    pickle.dump(sensor_seen,fw)
    fw.close()
