'''
Individual-level: source-space time-frequency (TFR) power for one vertex-ROI's
labels, for a two-condition comparison (cond1 vs cond2, e.g. dAT-seen stimulus vs
blank). Same as pipeline_tfr_source.py but the ROIs are the IIT-selected
vertex-ROIs (general_param['VertROI_params_dict']) instead of anatomical ROIs.
...
Consumed by pipeline_tfr_VertROI_group_band.py / pipeline_tfr_VertROI_group_ft.py
(and their _plot.py).

Config folder: /config_files/pipeline_tfr_VertROI/
    dAT_probe_stim_noft_nobc_deci5_nocut_vertFFstiVbla_1_30.json
    dAT_probe_stim_noft_nobc_deci5_nocut_vertFFstiVbla_30_100.json
...
if   flag_test:
    ...
    args.configfile = base / "config_files" / "pipeline_tfr_VertROI" / "dAT_probe_stim_noft_nobc_deci5_nocut_vertFFstiVbla_1_30.json"
