'''
Individual-level: compute source-space ERF (condition averages) for every VertROI
label of a subject, for the cond1 vs cond2 comparison.

Per subject + config:
  - build source estimates for all epochs (get_stcs_allsrc),
  - for each VertROI in VertROI_params_dict, read its label(s) and, per label,
    extract the label vertex time courses and run compute_all_combinations with
...
Consumed by pipeline_erf_VertROI_group.py.

Config folder: /config_files/pipeline_erf_VertROI/
    dAT_probe_stim_ft_nobc_deci1_nocut.json
...
if   flag_test:
    ...
    args.configfile = base / "config_files" / "pipeline_erf_VertROI" / "dAT_probe_stim_ft_nobc_deci1_nocut.json"
...
if  os.path.exists(pklfile_name) :
    ...
    for roi in roi_params.keys():
        ...
        if lb_use:
            print('----------get vertices information for',roi, list(lb_use.keys()))
            ## get vertno index
            label_vertidx_dict=\
            ...
            print('---------- No label is found for',roi)
            label_dict[roi] = {}
    ...
    for roi in roi_params.keys():
        ...
        for label_name, label in roilist_all[roi].items():
            # # ---------- validation ----------
            # # for vert ROI we could not check src as below for 
            # # vert ROI may not hae label in both hemi
            # label_src = np.sum(
            ...
            # # ---------- validation ----------
            # # for vert ROI we could not check src as below for we did not presave vidx for VertROI
