'''
Plot of the group-level band TFR analysis (pipeline_tfr_source_group_band.py):
band-averaged log power time courses per condition and their difference, for one
ROI, with significant clusters highlighted.

Run first: pipeline_tfr_source.py (individual) then
pipeline_tfr_source_group_band.py (group band CBPT + Bayesian).
...
Only plots; all statistics come from pipeline_tfr_source_group_band.py.

Config folders (same as pipeline_tfr_source_group_band.py):
  /config_files/pipeline_tfr_source_group_band_250_500/
      higher_gamma_250_500.json
      lower_alpha_250_500.json
  /config_files/pipeline_tfr_source_group_band_baseline/
...
if   flag_test:
    ...
    args.group_ana = 'pipeline_tfr_source_group_band_plot'
    args.group_configfolder = 'pipeline_tfr_source_group_band_baseline'
    args.group_configfile = base / "config_files" / "pipeline_tfr_source_group_band_baseline" / "lower_gamma_pre500_0.json"
...
for idx, subject in enumerate(sub_list):
    ...
    print(f'--------- load data for subject {idx+1} of {len(sub_list)} {subject}')
...
if 'baseline' in group_configfolder:
    ...
    times_use_w = [-0.5,1.5]
print('-------- use time window for plot ',times_use_w)
