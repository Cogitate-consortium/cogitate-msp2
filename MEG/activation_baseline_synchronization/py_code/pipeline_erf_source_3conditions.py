...
label of a subject, for a THREE-condition comparison (cond1 vs cond2 vs cond3).
...
Per subject + config:
  ...
  - for each ROI in roi_params, read its label(s) and, per label, extract the
    label vertex time courses and run compute_all_combinations_three with
    reducers (trial: mean & rms; vertex: rms) -> t_dict,
...
Consumed by pipeline_erf_source_3conditions_group.py.

Config folder: /config_files/pipeline_erf_source_3conditions/
    dAT_probe_face_blank_ft_bc_deci1_nocut.json
    dAT_probe_obje_blank_ft_bc_deci1_nocut.json
    dAT_probe_stim_blank_ft_bc_deci1_nocut.json
  each sets epoch_setting_name, epoch_data_name, compare_trltypes,
  compare_trltypes_select_column, cond1/cond2/cond3_content_name + _content_list.
'''
...
from _help_module import (compute_all_combinations_three,SourceROIEpochs
                              )
import numpy as np
import pandas as pd
...
if   flag_test:
    ...
    args.configfile = base / "config_files" / "pipeline_erf_source_3conditions" / "dAT_probe_stim_blank_ft_bc_deci1_nocut.json"
        ...
        for label_name, label in roilist_all[roi].items():
            ...
            if ('rh' in label.name) and ('lh' in label.name):
                ...
                label_src = np.sum(
                            [label.lh.restrict(src),
                                label.rh.restrict(src)])
                print('start ' ,roi,label_name,
                      'left ',len(label_src.lh.vertices),' right ',len(label_src.rh.vertices),
                    ' ',datetime.now(),)
            ...
            t_dict = compute_all_combinations_three(
                epochs= label_epochs,
