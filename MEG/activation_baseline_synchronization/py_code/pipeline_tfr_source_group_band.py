'''
Group-level (band): oscillatory power difference between two conditions in a
single frequency band, for one ROI, on the source-space TFR power computed by
pipeline_tfr_source.py.

Run first: pipeline_tfr_source.py (individual, per ROI + freq_range_name).

Loads each subject's per-label (vertices, freqs, times) power, then for the band
(band_name, e.g. alpha 8-13 Hz or gamma 60-90 Hz) and the group time window:
  - average power over the band frequencies and over vertices -> per-label (time,),
  - log10(cond1/cond2) power ratio per subject / label / time.
Statistics on the band- and label-averaged log10 power ratio:
  - Bayesian paired t-test (bayes_ttest, JZS r=0.707) per time point and on the
    time-window average -> bf10/bf01, saved as bf_*.pkl and csv/bf_*.csv;
  - cluster-based permutation test (permutation_cluster_1samp_test, tail from
    config, clustering over labels x time) on log10(cond1/cond2) -> cbpt_* pkl/csv.
The prereg specifies the band power difference is tested with a cluster-based
permutation test; the Bayesian t-test is reported alongside it.

Reduction of each label's (vertices, freqs, times) power for the test:
  vertices     -> averaged over all label vertices,
  frequencies  -> averaged over the one band's frequencies (collapsed),
  times        -> kept.
So each label is a 1D time course and clustering is over labels x time.
This is the only difference from pipeline_tfr_source_group_ft.py, which instead
keeps the full frequency axis (vertices -> averaged; frequencies -> kept;
times -> kept) and clusters over labels x freqs x times.

Config folders:
  /config_files/pipeline_tfr_source_group_band_250_500/   (250-500 ms window)
      higher_gamma_250_500.json
      lower_alpha_250_500.json
  /config_files/pipeline_tfr_source_group_band_baseline/   (pre-stimulus baseline)
      higher_alpha_pre500_0.json
      lower_gamma_pre500_0.json
  each sets band_name, cond1/cond2_timewindow, tail, cbpt_params_name.
...
if   flag_test:
    ...
    args.group_ana = 'pipeline_tfr_source_group_band'
    args.group_configfolder = 'pipeline_tfr_source_group_band_250_500'
    args.group_configfile = base / "config_files" / "pipeline_tfr_source_group_band_250_500" / "higher_gamma_250_500.json"
...
# --- data with select the statistic time window; selected band frequency
bandpower1_sub_label_t =  \
    np.stack([
    np.stack([
        np.nanmean(
             # (vertices, freqs, time): select bandfreq
            sub_dict[sub][lab][cond1_content_name][:, band_fidx, :][:, :, cond1_tidx], 
...
bandpower2_sub_label_t =  \
    np.stack([
    np.stack([
        np.nanmean(
             # (vertices, freqs, time): select bandfreq
            sub_dict[sub][lab][cond2_content_name][:, band_fidx, :][:, :, cond2_tidx], 
            axis=(0, 1) # → average over vertices & freq → (time,)
        )
        for lab in labels
    ], axis=0)  # → (n_labels, n_time)
...
conddif_sublabelt_bandpower  =np.log10(
                            bandpower1_sub_label_t  /\
                            bandpower2_sub_label_t )
...
sublabelt_bandpower=  \
    ...
    np.stack([
        np.log10(
             # (vertices, freqs, time): select bandfreq
    np.nanmean(sub_dict[sub][lab][cond1_content_name][:, band_fidx, :][:, :, cond1_tidx], axis=(0, 1))/ # → average over vertices & freq → (time,)/
    np.nanmean(sub_dict[sub][lab][cond2_content_name][:, band_fidx, :][:, :, cond2_tidx], axis=(0, 1))
...
cond1_subt = np.log10(np.mean(bandpower1_sub_label_t,axis=1 ))
cond2_subt = np.log10(np.mean(bandpower2_sub_label_t,axis=1 ))
...
if ((os.path.exists(clusterfile_name)) or  (os.path.exists(blankfile_name)) ) &  (os.path.exists(cbpt_result_name)):
    ...
    X_observ_clulsterdim =  conddif_sublabelt_bandpower
