...
across the full frequency x time plane, for one ROI, on the source-space TFR
power computed by pipeline_tfr_source.py.
...
Statistics on the label-averaged log10 power ratio:
  - Bayesian paired t-test (bayes_ttest, JZS r=0.707) per freq x time point,
    saved as bf_*.pkl;
...
This is the only difference from pipeline_tfr_source_group_band.py, which instead
averages over one band's frequencies (vertices -> averaged; frequencies -> collapsed;
times -> kept) and clusters over labels x time.
...
cond1_subft = np.log10(np.mean(bandpower1_sub_label_f_t,axis=1 ))
cond2_subft = np.log10(np.mean(bandpower2_sub_label_f_t,axis=1 ))
