'''
Individual-level synchronization analysis: IIT prediction 1 and 2

Configure file folder:
/config_files/pipeline_syn_ppc_PCA_VertROI/
...
load time courses for the different ROIs (calculated in pipeline_syn_timecourse_PCA_VertROI.py)
...
(mne spectral_connectivity_epochs, method='ppc'). This is the connectivity
measure specified in the preregistration.
NOTE: this is the IIT test, using PCA vertex time courses (category-selective
vertices vs V1/V2 responsive vertices). The GNW test (GED spatial filters,
FFA vs PFC) is in pipeline_syn_subsampling_ppc_GED_ROI.py (PPC) and
pipeline_syn_subsampling_dfc_GED_ROI.py (MI-based DFC).
...
(This PCA_VertROI script only uses the vert* / PCA time courses
 (pipeline_syn_timecourse_PCA_VertROI).
 The GED entries (FFface/FFobje/PFCact) are only used by the GED_ROI scripts.)
...
if   flag_test:
    ...
    args.configfile = base / "config_files" / "pipeline_syn_ppc_PCA_VertROI" / "noft_nobc_deci1_nocut_allseen.json"
...
ana_nosubsampling= 'pipeline_syn_ppc_PCA_VertROI'
...
for ROIpairs_name, ROIpairs in param['ROIpairs_dict'].items():
    ...
    # the time series resulting from PCA on the selected vertices
    # (i.e. results of pipeline_syn_timecourse_PCA_VertROI.py)
