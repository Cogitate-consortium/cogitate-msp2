...
if [ $# -ne 4 ];
    ...
    echo "sbatch --array=101,103,105 srun_preproc.sh SA V1 run 1"
    echo "Exiting."
fi
...
srun python P99_run_preproc.py --sub ${sub_prefix}`printf "%03d" $SLURM_ARRAY_TASK_ID` --visit ${visit} --record ${record} --step ${step}