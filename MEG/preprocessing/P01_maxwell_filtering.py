...
def run_maxwell_filter(subject_id, visit_id, record="run"):
    ...
    for fname in sorted(os.listdir(data_path)):
        if fname.endswith(".json") and record in fname:
            ...
            elif "rest" in fname or "noise" in fname:
                run = None
            print("  Run: %s" % run)
            ...
            if run == "01":
                ...
                if visit_id == "V1":
                    ...
                    raise ValueError("Error: resting-state visit not valid")
                
                # Read task's run1 raw
                bids_path_task = mne_bids.BIDSPath(
                    root=bids_root, 
                    subject=subject_id,  
                    datatype='meg',  
                    task=task,
                    ...
                    session=visit_id, 
                    extension='.fif')
                raw_task = mne_bids.read_raw_bids(bids_path_task)
                
                # Set destination
                destination = raw_task.info['dev_head_t']
